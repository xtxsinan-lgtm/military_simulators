...
struct CombatRadiusView: View {
    ...
    private func dashPanel(_ r: CombatRadiusResult) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("表尾「实用最大巡航速度」在 Ma 1.2 以上取最佳巡航高度尚未从峰值回落时的上限；「最大巡航速度」允许掉到 11 km。最佳巡航高度使升阻比×总效率最大。最大 L/D 为可飞高度（军推优先，不足则加力）中升阻比最大的点；加力可飞按全部加力，高度可到海平面。极速按阻力等于全部加力（不留巡航裕度）。混合作战半径仅超音速：去程该马赫、返程 Ma 0.8。")
                .font(.system(size: 10, design: .monospaced))
                .foregroundStyle(CombatRadiusTheme.textDim)
        }
    }
