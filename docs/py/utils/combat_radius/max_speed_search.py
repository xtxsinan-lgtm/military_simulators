...
约束：平飞阻力 ≤ 该高度全部加力可用推力（默认 100%，不留巡航 8% 裕度）。
每个马赫只在能飞的高度里选升阻比最大的点（阻力最小），再比较真速。
推力由海平面加力标定经理想循环外推；空战重量与作战半径巡航一致。

不加巡航裕度的原因：F-35C 公开包线约 Ma 1.6，模型在 Ma 1.5 已经推阻拉平，
4–8% 裕度会让该点加力不可飞，且光滑隐身机只会降高、马赫几乎不掉。
F-22 公开极速 Ma 2.25 由超巡带之后的体积波阻收住，而不是靠预留推力。
"""
...
MACH_REFINE = 0.02
KTS_PER_MPS = 1.94384
# 加力极速仍按推阻拉平。巡航 8% 裕度几乎不降 F-22 马赫（只是飞得更低），
# 却会让 F-35C 的 Ma 1.5 加力不可飞；F-22 公开 Ma 2.25 靠后段体积波阻标定。
