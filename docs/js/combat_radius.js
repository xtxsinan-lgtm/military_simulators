...
function renderDash(r, sourceLabel) {
  ...
  $('dashBox').innerHTML = `
    ...
    <p class="note">${sourceLabel} 最佳 L/D 指该马赫下使升阻比×总效率最大的高度。低马赫爬高会因负载过大降低总效率，大迎角也会压低升阻比；Ma 1.5 以前最佳高度随速度升高。表尾「实用最大巡航速度」是最佳巡航高度尚未从峰值回落时的上限；「最大巡航速度」允许掉到 11 km。最大 L/D 为可飞高度（军推优先，不足则加力）中升阻比最大的点；加力可飞按全部加力（不留巡航裕度），高度可到海平面。极速按阻力等于全部加力，各马赫取最大升阻比后再取真速最大点。混合作战半径仅超音速：去程该马赫、返程 Ma 0.8。</p>
  `;
