...
def test_get_api_data_returns_aircraft_and_carriers():
    ...
    assert 'combat_radius_results' in data
    ...
    assert any(p['id'] == 'Typhoon' for p in data['combat_radius_presets'])
    assert any(p['id'] == 'Su-75' for p in data['combat_radius_presets'])
    takeoff_ids = {a['id'] for a in data['aircraft']}
